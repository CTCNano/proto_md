...
class ContinuumSubsystem(subsystems.SubSystem):
    ...
    def translate(self, dCG):
        ...
        self.atoms.positions = self.FineScale(
    ...
    def frame(self):
        ...
        if self.NumNodes() < self.nNodes:
            ...
            CG[: self.NumNodes()], CGv[: self.NumNodes()], CGf[: self.NumNodes()] = (
                self.ComputeCG_Pos(pos),
                self.ComputeCG_Vel(pos, vel),
                self.ComputeCG_For(pos, vel, forces),
            )
        else:
            CG, CGv, CGf = (
                self.ComputeCG_Pos(pos),
    ...
    def ComputeCG_For(self, pos, vel, forces):
        return self.CppFV.Py_ComputeCG_For(
            pos, vel, forces, self.CppFV.GetAdjNumNodes()
        )
    ...
    def FineScaleVelo(self, r, Vels, Mom):
        ...
        self.PetscError, Vels = self.CppFV.Py_FineGrainMom(
            Mom.copy(),
            Mom.copy(),
            Mom.copy(),
            Vels[:, 0],
            ...
            Vels[:, 2],
            r,
            self.atoms.n_atoms * Vels.shape[1],
        )
    ...
    def FineScale(self, Coords, CG):
        ...
        self.PetscError, Coords = self.CppFV.Py_FineGrain(
            CG,
            Coords[:, 0],
            Coords[:, 1],
            Coords[:, 2],
            self.atoms.n_atoms * Coords.shape[1],
            self.Assemble,
        )
...
def ContinuumSubsystemFactory(system, selects, **args):
    ...
    if (
        "NumNodes_x" in args
        and "NumNodes_y" in args
        and "NumNodes_z" in args
        and "Resolution" in args
    ):
        pass
    else:
    ...
    return (
        args["NumNodes_x"] * args["NumNodes_y"] * args["NumNodes_z"],
        [ContinuumSubsystem(system, select, **args) for select in selects],
    )
