...
def make_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="python -m proto_md",
        description="proto_md - coarse grained molecular dynamics",
    )
    ...
    ap = subparsers.add_parser(
        "config",
        help="create a simulation database, from which a simulation can be run",
    )
    ...
    ap.add_argument(
        "-top_args",
        default={"ff": "charmm27", "protein": "protein"},
        type=eval,
    ...
    ap.add_argument(
        "-subsystem_selects",
        dest="subsystem_selects",
        required=False,
        nargs="+",
        default=["not resname SOL"],
    ...
    ap.add_argument(
        "-integrator",
        default="proto_md.integrators.FactorizationIntegrator",
    ...
    ap.add_argument("-cg_steps", default=10, help="number of coarse grained time steps")

    ap.add_argument(
        "-dt", default=0.1, help="size of coarse grained time step in picoseconds"
    )
    ...
    ap.add_argument("-multi", default=1, help="number of parallel MD runs")

    ap.add_argument(
        "-eq_steps", default=10, help="number of timesteps to run equilibriation"
    )

    ap.add_argument(
        "-mn_args",
        default=config.DEFAULT_MN_ARGS,
    ...
    ap.add_argument(
        "-eq_args",
        default=config.DEFAULT_EQ_ARGS,
        type=eval,
        help="equilibration (GROMACS) config file",
    )
    ap.add_argument(
        "-md_args",
        default=config.DEFAULT_MD_ARGS,
        type=eval,
    ...
    ap.add_argument(
        "-center",
        action="store_true",
        help="centers the system in the supplied box before beginning simulation",
    )
    ...
    ap.add_argument(
        "-debug",
        action="store_true",
        help="enable debug mode (save all simulation directories).",
    )
    ...
    ap = subparsers.add_parser(
        "top",
        help="Given a atomic structure file, auto-generate a topology, "
    ...
    ap = subparsers.add_parser(
        "solvate",
        help="Given a atomic structure file, attempt auto-solvation, "
        "and save it in a directory. "
        "This is usefull for testing auto-solvation. "
        "All the inputs to solvate are returned from top",
    )
    ap.add_argument(
        "-o", help="output directory where solvated structure will be generated"
    )
    ap.add_argument(
        "-struct",
        dest="struct",
    ...
    ap.add_argument(
        "-top", dest="top", required=True, help="name of the topology file."
    )
    ap.add_argument(
        "-box",
        dest="box",
    ...
    ap = subparsers.add_parser(
        "run",
        help="start or continue a full simulation. "
    ...
    ap = subparsers.add_parser(
        "runsol",
        help="Like run, but perfoms only the auto-solvation step. "
    ...
    ap.add_argument("sys", help="name of simulation file")
    ap.add_argument(
        "-sol", action="store_true", help="auto solvate before minimization"
    )
    ap.add_argument(
        "-debug",
        action="store_true",
    ...
    ap = subparsers.add_parser(
        "mneq", help="performs energy minimization followed by equilibriation"
    )
    ap.add_argument("sys", help="name of simulation file")
    ...
    ap = subparsers.add_parser(
        "mneqmd",
        help="performs energy minimzatin, equilibriation and molecular dynamics",
    )
    ap.add_argument("sys", help="name of simulation file")
    ap.add_argument("-sol", action="store_true", help="auto solvate between steps")
    ap.add_argument(
    ...
    ap = subparsers.add_parser(
        "md", help="perform ONLY an molecular dynamics step with the starting structure"
    )
    ap.add_argument("sys", help="name of simulation file")
    ...
    ap = subparsers.add_parser(
        "cg_step", help="perform just the coarse grained portion of the time step"
    )
    ap.add_argument("sys", help="name of simulation file")
    ap.add_argument(
        "-debug",
