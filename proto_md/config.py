    ...
    with h5py.File(fid, "w") as hdf:
        ...
        try:
            if box:
                ...
                box = universe.trajectory.ts.dimensions[:3]
                print(
                    "using pdb specified periodic boundary conditions: {}".format(box)
                )
            conf[BOX] = box
        except Exception as e:
        ...
        if top is None:
            ...
            with md.topology(struct=struct, protein="protein") as top:
                ...
                if should_solvate:
                    ...
                    print("attempting auto solvation...")
                    with md.solvate(
                        top_includes=top_includes.values(),
                        box=box / 10.0,
                        concentration=concentration,
                        anion=anion,
                        cation=cation,
        ...
        try:
            ...
            print(
                "created subsystem factory {}, attempting to create subsystems...".format(
                    subsystem_factory
                )
            )
...
def set_tempdir(f):
    ...
    if not os.path.isdir(d):
        raise ValueError(
            "Error, the file {} is not located in a valid directory {}".format(f, d)
        )
