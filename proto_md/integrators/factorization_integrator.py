...
class FactorizationIntegrator(integrator.Integrator):
    def cg_step(self):
        """
        Performs a forward euler step (the most idiotic and unstable of all possible integrators)
        to integrate the state variables. Reads self.cg_momenta to advance the cg state variables.
        ...
        avg_velocities = avg_velocities.flatten()[:, numpy.newaxis]
