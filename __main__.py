...
Last Modified Feb 17, 2014 by Andrew
...
def make_parser():
    ...
    parser = argparse.ArgumentParser(prog="python -m proto_md", description="protoMD - a prototyping toolkit for multiscale MD")
    ...
    ap.add_argument("-subsystem_factory", dest="subsystem_factory", required=False, 
                    default="proto_md.subsystems.SpaceWarpingSubsystemFactory",
                    help="fully qualified function name which can create a set of subsystems, "
                         "defaults to SpaceWarpingSubsystemFactory")
    ...
    ap.add_argument("-subsystem_args", dest="subsystem_args", required=False, 
                    ...
                    help="a list of additional arguments passed to the subsystem factory, "
                         ...
                         ""
                         "The most comonly used subsystem is proto_md.subsystems.SpaceWarpingSubsystemFactory, "
    ...
    ap.add_argument("-cg_steps", default = 1000,
                    help="number of coarse grained time steps")
    
    ap.add_argument("-dt", default=1.0, 
                    help="size of coarse grained time step in picoseconds")
    
    ap.add_argument("-mn_steps", default = 5000,
                    help="number of MD steps to take when performing a minimization")
    
    ap.add_argument("-md_steps", default=100,
                    help="number of MD steps to take when performing the MD runs")
    
    ap.add_argument("-multi", default=1,
                    help="number of parallel MD runs. This is useful if doing ensemble MD.")
    
    ap.add_argument("-eq_steps", default=1000, 
                    help="number of timesteps to run equilibriation")

    ap.add_argument("-mn_args", default=config.DEFAULT_MN_ARGS, type=eval,
                    help="minimization (GROMACS) config file")

    ap.add_argument("-md_args", default=config.DEFAULT_MD_ARGS, type=eval,
		    help="md (GROMACS) config file")
    ...
    ap.add_argument("-eq_args", default=config.DEFAULT_EQ_ARGS, type=eval,
		    help="equilibration (GROMACS) config file")
    ...
    ap.add_argument("-constraints", dest="constraints", action="store_true",
                    help="the microstate is constructed with correct bond lengths and angles."
                    "defaults to False. This is a boolean flag, to enable just add \'-constraints\' with no args.")
    ...
    ap.add_argument("-minimize", action="store_true", help="perform energy minimization every CG time step")    
    ...
    def analyze(struct, traj, var, ofname, plot, subsystem_factory, subsystem_selects, subsystem_args, N_CG_plot, nframes):
        analysis.Plot_Time_Integral(struct, traj, var, ofname, subsystem_factory, plot, subsystem_selects, subsystem_args, N_CG_plot, nframes)
    ...
    ap.add_argument("-var", dest="var", required=False, default='velocities', help="which variables to use: velocities "
                    "(non-inertial) or forces (inertial), or even both")
    ap.add_argument("-N_CG_plot", dest="N_CG_plot", required=False, help="number of CG integrals to plot", type=int, default=None)
    ap.add_argument("-nframes", dest="nframes", required=True, help="number of frames storedi in the traj file", type=int, default=-1)
    ap.add_argument("-ofname", dest="ofname", required=False, default='tmp', help="output filename", type=str)
    ap.add_argument("-subsystem_selects", dest="subsystem_selects", nargs="+", default=["not resname SOL"], help="selection")
    ap.add_argument("--plot", action="store_true", required=False, help="plots the integral as a function of \delta")
    ap.add_argument("-subsystem_args", dest="subsystem_args", required=True,
                    type=eval,
                    help="a list of additional arguments passed to the subsystem factory, "
                         "the first item of the list may be the string \'resid unique\', which "
                         "creates a separate subsystem for each residue. "
                         ...
                         "subsystem for each residue." )
    ap.add_argument("-subsystem_factory", dest="subsystem_factory", required=False,
                    default="proto_md.subsystems.ContinuumSubsystemFactory",
                    help="fully qualified function name which can create a set of subsystems, "
