...
class MDManager(dict): 
	...
	if self.has_key("dirname_tpr"):
	    self.dirname_tpr = self["dirname_tpr"]
            del self["dirname_tpr"]
	else:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
		...
		logging.error("MDManager will NOT delete directory {}".format(self.dirname_tpr))
...
class MDrunner(gromacs.run.MDrunner):
	...
	if os.environ.get("DMS_NPROCS", None) is not None:
	    ...
	    fp.close()
            logging.debug("determined nprocs is {} from PBS_NODEFILE".format(nprocs))
            return ["mpirun", "-n", str(nprocs)]
...
def setup_md(struct, top, top_includes, deffnm="md", dirname=None, mdp="md_CHARMM27.mdp", 
             mainselection=None, dirname_tpr=None, equilibriation=True, **kwargs):
    ...
    :Keywords:
       ...
       *dirname_tpr*
	  name of the directory containing the md.tpr file from the previous CG time step 
       *kwargs*
    ...
    if dirname_tpr is None or equilibriation is True:
	logging.info('tpr file will be generated.')
    else:
	logging.info('DMS will attempt to copy tpr file from the previous timestep')

    if dirname is None:
        dirname = tempfile.mkdtemp(prefix="tmp." + deffnm + ".")
        logging.debug("created md dir {}".format(dirname))
    
    top = data_tofile(top, "src.top", dirname=dirname) # This should be deleted for step >= 2

    # Store previous md.gro file in tmp.tpr dir ... GOOD IDEA?!!!
    # For solvated syste,s and CG timestep > 0 , this is useless
    struct = data_tofile(struct, "src.gro", dirname=dirname)
    ...
    # this stops grompp from failing if there are warning. 
    # TODO, is this the best place to put this, should this be in in 
    # config.py ????
    kwargs.setdefault('maxwarn', -1)
        
    logging.info('DMS equilibriation is set to {}'.format(equilibriation))
    logging.info("using mdp template {} from key {}".format(config.templates[mdp], mdp))
    mdp = config.templates[mdp]

    if struct is None:
	# This means solvation is True and CG time step >= 1
	# For solvated system, struct is NOT None for the zeroth time step!
	# dirname_tpr must NOT be None! Do a sanity check

	if dirname_tpr is None:
		logging.info('dirname_tpr is None for CG time step > 0. Something is SERIOUSLY wrong!')
	else:
		struct = data_tofile("{}/md.gro".format(dirname_tpr), "src.gro", dirname=dirname)

	logging.debug("calling _setup_MD with kwargs: {}".format(kwargs))
        setup_MD = gromacs.setup._setup_MD(dirname, struct=struct, top=top, deffnm=deffnm, mdp=mdp, mainselection=mainselection,  **kwargs)
	    ...
	    data_tofile("{}/md.tpr".format(dirname_tpr), "md.tpr", dirname=dirname)

	    logging.info('writing {}/src.trr from {}/src.gro'.format(dirname,dirname))
	    trjconv = gromacs.tools.Trjconv(f='{}/src.gro'.format(dirname), o='{}/src.trr'.format(dirname))
	    check_failure = trjconv.run()
				
	    logging.info('updating md.tpr from src.trr')
	    tpbconv = gromacs.tpbconv(s='{}/md.tpr'.format(dirname), f='{}/src.trr'.format(dirname),o='{}/md.tpr'.format(dirname))

	    setup_MD = {'dirname':dirname, 'deffnm':'md'}

    if dirname_tpr is not None:
	logging.debug("deleting {}".format(dirname_tpr))
        shutil.rmtree(dirname_tpr)

    dirname_tpr = tempfile.mkdtemp(prefix="tmp.tpr." + deffnm + ".")
    setup_MD["dirname_tpr"] = dirname_tpr
    _ = data_tofile("{}/md.tpr".format(dirname), "md.tpr", dirname=dirname_tpr)
    
    return MDManager(setup_MD)

def topology(struct, protein="protein", dirname="top", ff="gromos43a1", water="spc", ignh=True, **top_args):
    """
    ...
    pdb2gmx_args = {"ff":ff, "water":water}
...
def solvate(struct, top, top_includes=None, box = None,
            concentration=0.1, cation='NA', anion='CL',
...
def run_md(dirname, dirname_tpr, md_runner=MDrunner, **kwargs):
    """
    ...
    # figure out what the output file is, try set default output format to gro
    structs = None
    ...
    # Save md.gro from the last MD run in tmp.md to tmp.md.tpr. This will be used to update the coords
    _ = data_tofile("{}/md.gro".format(dirname), "md.gro", dirname=dirname_tpr)
...
def check_main_index(struct):   
    ...
    with gromacs.utilities.in_dir(dirname):
        writer = MDAnalysis.Writer("src.gro")
        writer.write(struct)
        del writer
        groups = gromacs.setup.make_main_index("src.gro")
