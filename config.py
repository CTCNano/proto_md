...
from numpy import array, fromfile, uint8, all, concatenate, linalg
import tempfile
...
def _generate_template_dict(dirname):
    ...
    resource_listdir lists all files/dirs in dirname
    """
...
TODO, this should be cleaned up: if a user specifies an mdp template, then that template
...
TOPOL_TOP = "topol.top"
TOPOL_TOP_SOLV = "topol_solvated.top"
TOPOL_INDICES = "topol_indices"
TOPOL_EQ_DIST = "topol_eq_dist"
...
DEFAULT_MD_ARGS = { "mdp":"md.mdp",    # the default mdp template 
                    "nstxout": 50,     # trr pos
                    "nstvout": 50,     # trr veloc
                    "nstfout": 50,     # trr forces
                    "dt":      0.001,  # trr time step (1fs = 0.001ps)
		    "gen_vel": "no",   # important for reading velocities from previous MD phase
		    "nstlog": 100
                    }

DEFAULT_EQ_ARGS = { "mdp":"eq.mdp",  # the default mdp template 
                    "define":"-DPOSRES", # do position restrained md for equilibriation
                    "dt": 0.001 # trr time step (1fs = 0.001ps)
                    }


def create_top(o, struct, posres, **args):
    print("attempting to auto-generate a topology...")
    
    top = md.topology(struct=struct, protein="protein", posres=posres, dirname=o, **args)
        # topology returns:
...
def create_sim(fid,
               ...
               top = None,
               temperature = 300,
               subsystem_factory = "dms21.subsystems.RigidSubsystemFactory",
               subsystem_selects = ["not resname SOL"],
               subsystem_args = [],
               integrator = "dms21.integrators.LangevinIntegrator",
               integrator_args = [],
               cg_steps = 10,
               dt  = 0.1,
               multi = 1,
    ...
    with h5py.File(fid, "w") as hdf:
	    ...
	    if all(box >= 1.1 * bbox):
                ...
                trans = box / 2.0 - universe.atoms.centerOfMass() 
	...
	attr(CONSTRAINTS, int, constraints)
	...
	# Set mdp include dir
        ...
        if top is None:
            print("attempting to auto-generate a topology...")
            with md.topology(struct=struct, **(kwargs['top_args'])) as top:
                # topology returns:
                # {'top': '/home/andy/tmp/Au/top/system.top', 
                # 'dirname': 'top', 
                # 'struct': '/home/andy/tmp/Au/top/protein.pdb'}
                ...
                if should_solvate:
                    # convert Angstrom to Nm, GROMACS works in Nm, and
                    # we use MDAnalysis which uses Angstroms
                    print "attempting auto solvation...", concentration
                    with md.solvate(top_includes=top_includes.values(), box=box/10.0, concentration=concentration, anion=anion, cation=cation, **top):
                        # solvate returns 
                        # {'ndx': '/home/andy/tmp/Au/solvate/main.ndx', 
                        # 'mainselection': '"Protein"', 
                        # 'struct': '/home/andy/tmp/Au/solvate/solvated.pdb', 
            ...
            if should_solvate:
                ...
                with md.solvate(struct=struct, top=top, top_includes=top_includes.values(), 
                                box=box/10.0, mainselection=mainselection, o='solvated.gro', 
				concentration=concentration, anion=anion, cation=cation):
                    # solvate returns 
                    # {'ndx': '/home/andy/tmp/Au/solvate/main.ndx', 
                    # 'mainselection': '"Protein"', 
                    # 'struct': '/home/andy/tmp/Au/solvate/solvated.pdb', 
        ...
        try:
            ...
            test_ss, ncgs = factory(dummysys, subsystem_selects, **subsystem_args)
            
            print("subsystem factory appears to work, produces cg variables for each {} subsystems.".format(len(test_ss)))
	...
	if constraints:
        	# For now keep this stupid inefficient I/O until aprun mode works for DMS

        	bonds, angles = util.Read_Top(top)
        	indices = concatenate((bonds, angles))

        	leq = linalg.norm(universe.atoms.positions[indices[:,0]] - universe.atoms.positions[indices[:,1]], axis=1)**2.0

        	hdf['src_files'].create_dataset(TOPOL_INDICES, shape=indices.shape, data=indices)
        	hdf['src_files'].create_dataset(TOPOL_EQ_DIST, shape=leq.shape, data=leq)
