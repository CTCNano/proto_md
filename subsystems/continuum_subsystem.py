...
class ContinuumSubsystem(subsystems.SubSystem):
    ...
    def universe_changed(self, universe):
        ...
        """
        self.atoms = universe.selectAtoms(self.select) # handles SS selection [O only]
	self.universe = universe # Handles global system [+Hs]
...
def ContinuumSubsystemFactory(system, selects, **args):
    ...
    return [ContinuumSubsystem(system, select, **args) for select in selects], args['NumNodes_x'] * args['NumNodes_y'] * args['NumNodes_z']
