...
from numpy import array, zeros, reshape, conjugate, \
                  real, max, fromfile, uint8, savetxt
import numpy.random
...
class System(object):
    ...
    __slots__ = ["hdf", "config", "universe", "_box", "ncgs", "subsystems", "cg_positions", "cg_velocities", "cg_forces", "numatoms", "atomic_positions", "atomic_velocities", "atomic_forces", "dim", "nrs"]
	...
	self.atomic_forces     = [zeros((md_nensemble,ss.atoms.numberOfAtoms(),3)) for ss in self.subsystems]
        ...
        #logging.info("pos {}".format(self.atomic_positions.shape))
        #logging.info("frc {}".format(self.atomic_forces.shape))
        #logging.info("vel {}".format(self.atomic_velocities.shape))
    ...
    def v_translate(self, cv_translate):
        """
	...
	# make sure it is of the correct shape, this will make a 1D array out of the
        ...
        for i, s in enumerate(self.subsystems):
            s.v_translate(cv_translate[i], self.config[DT])
	...
	self.ncgs = self.subsystems[0].GlobNumNodes
    ...
    def md(self, struct=None, top=None, sub=None, **args):
        ...
        with self.setup_md(struct, top) as mdsetup:
            ...
            self._processes_trajectories(mdres.structs[0], sub)

    def _processes_trajectories(self, struct, sub=None):
        """
	...
	self.cg_positions  = zeros((self.nrs,self.ncgs))
	self.cg_velocities = zeros((self.nrs,self.ncgs))
	self.cg_forces	   = zeros((self.nrs,self.ncgs))

        # save the universe to a tmp file to load back once the trajectories 
        # are processed.


	# First save the previous CG positions + velocities
	for si, s in enumerate(self.subsystems):
        	pos, vel, force = s.frame()
                self.cg_positions[si] = pos
                self.cg_velocities[si] = vel
        ...
        with tempfile.NamedTemporaryFile(suffix=".gro") as tmp:  
            ...
            del writer
            numframe = 0
            # universe is saved, process files from MD
                
	    self.universe.load_new(struct[0], sub=sub)
	    for si, s in enumerate(self.subsystems):
		pos, vel, force = s.frame()

            	self.cg_positions[si] = pos - self.cg_positions[si]
            	self.cg_positions[si] /= (self.config[MD_STEPS] * 0.001)

		self.cg_velocities[si] = vel - self.cg_velocities[si]
            	self.cg_velocities[si] /= (self.config[MD_STEPS] * 0.001)

            # done with MD files, load original contents of universe back
    ...
    def solvate(self):
        ...
        """
        return md.solvate(self.universe, self.top, top_includes = self.top_includes, mainselection=self.mainselection)
